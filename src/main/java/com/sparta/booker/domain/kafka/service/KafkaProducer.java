	...
	public ResponseEntity<EventResponseDto> produceMessage(Long eventId, User user) {
		...
		if (!isSuccess) {
			...
			String eventMessage = String.format("{\"eventId\": %d, \"userId\": \"%s\", " +
					...
					eventId, user.getId(), applicationDate, applicationTime);
			...
			ListenableFuture<SendResult<Long, String>> listenableFuture = kafkaTemplate.send("book-1", eventId, eventMessage);
